...
def print_header(config: CFRTrainerConfig, preset: str) -> None:
    ...
    print("- `info_set_count` 表示当前策略表学到了多少种局面。")
    print()
...
def print_history(history: List[dict]) -> None:
    ...
    for item in history:
        ...
        print(
            ...
            f"验证胜率 {win_rate * 100:>5.1f}% |{win_bar}|  信息集 {item['info_set_count']}"
        )
    print()
    print("* 代表这一轮刷新了最佳检查点")
    print()
...
def print_takeaways(summary: dict) -> None:
    ...
    print(f"信息集数量       : {summary['info_set_count']}")
    if "run_dir" in summary:
        print(f"结果目录         : {summary['run_dir']}")
    print()
    print("训练机制说明：")
    print("- 每轮先抽一手牌，再用 regret matching 在整棵下注树里回传后悔值。")
    print("- 策略不是挑一个“冠军个体”，而是持续累积平均策略表。")
    print("- 检查点会把当前平均策略拿去打训练池和验证局，方便你判断有没有真的变强。")
    print()
...
def print_tuning_tips() -> None:
    ...
    print("- 如果曲线太抖：增大 `--eval-repeats` 和 `--validation-repeats`。")
    print()
